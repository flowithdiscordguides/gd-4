...
- Research does not authorize dependency installation, Git operations, or any other action prohibited by the user
  laws or project context.

## Standard 8 — Categorical Neighbors and Feature Integration

Before implementing a feature, setting, control, workflow, or visual behavior, search the current source and relevant
task history for categorical neighbors. Search by purpose and implementation shape, not only by the user's exact name.

- Inspect analogous settings, components, controls, routes, state, handlers, persistence, validation, error handling,
  permissions, styling, design tokens, assets, tests, registration points, and documentation.
...
- Record the categorical neighbors found and the evidence-based reuse or separation decision in task progress before
  editing. When the finding is reusable across future tasks, preserve it in learned history.
- During verification, confirm the new work did not duplicate an existing capability, fork shared styling without a
  reason, bypass a common validation or persistence path, or regress an adjacent feature that uses the same system.
...
| --- | --- | --- | --- |
| [Feature, component, handler, state, persistence, styling, or prior fix] | [What must remain true] | [Reuse, extension, or deliberate separation] | [Source trace and permitted check] |
...
- After editing, verify each row against current source and every permitted runtime or static check. Record the result
  in progress history; an unverified row prevents a completion claim.
